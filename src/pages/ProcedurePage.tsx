...
import { ArrowRight, FileText, AlertTriangle, CheckCircle2, Clock, DollarSign } from "lucide-react";
import SeoSchema from "@/components/SeoSchema";
import AuthorByline from "@/components/AuthorByline";
import EligibilityForm from "@/components/EligibilityForm";
import { getProcedure, PROCEDURES_LIST } from "@/data/procedureData";
...
const ProcedurePage = () => {
  ...
  const g = getProcedure(topic);
  if (!g) return <Navigate to="/" replace />;

  const canonical = `${SITE}/procedure/${g.slug}`;
  const headline = `${g.name} (${YEAR})`;
  const metaTitle = `${g.shortName} ${YEAR} — How-To, Timing & Tips | 4 Aces Visa`;
  ...
  const others = PROCEDURES_LIST.filter((x) => x.slug !== g.slug);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <SeoSchema breadcrumbs={[
        { name: "Procedures", url: "/procedure/biometrics-collection" },
        { name: g.shortName, url: `/procedure/${g.slug}` },
      ]} />
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <FileText className="h-3.5 w-3.5" /> IRCC Procedures
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/procedure/${g.slug}`} /></div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 text-center">
              <h2 className="text-2xl font-bold text-foreground md:text-3xl">Need help with this step?</h2>
              <p className="mt-2 text-muted-foreground">Sahil Garg's team handles IRCC procedures every day. Book a free consult.</p>
            </div>
            <EligibilityForm sourcePage={`procedure-${g.slug}`} heading={g.shortName} />
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-foreground"><CheckCircle2 className="h-5 w-5 text-primary" /> More procedural guides</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/procedure/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
