...
import { ArrowRight, Plane, AlertTriangle, CheckCircle2, Clock, DollarSign } from "lucide-react";
import SeoSchema from "@/components/SeoSchema";
import AuthorByline from "@/components/AuthorByline";
import EligibilityForm from "@/components/EligibilityForm";
import { getVisit, VISITS_LIST } from "@/data/visitCanadaData";

const SITE = "https://www.gargbrothers.ca";
const YEAR = new Date().getFullYear();

const VisitCanadaPage = () => {
  const { topic } = useParams<{ topic: string }>();
  const g = getVisit(topic);
  if (!g) return <Navigate to="/" replace />;

  const canonical = `${SITE}/visit-canada/${g.slug}`;
  const headline = `${g.name} (${YEAR})`;
  const metaTitle = `${g.shortName} ${YEAR} — Apply, Documents & Tips | Garg Brothers`;
  ...
  const others = VISITS_LIST.filter((x) => x.slug !== g.slug);
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <Plane className="h-3.5 w-3.5" /> Visit Canada
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/visit-canada/${g.slug}`} /></div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 text-center">
              ...
              <p className="mt-2 text-muted-foreground">Our team handles visitor visas and Super Visas every week. Book a free consult.</p>
            </div>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-foreground"><CheckCircle2 className="h-5 w-5 text-primary" /> More visitor pathways</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/visit-canada/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
