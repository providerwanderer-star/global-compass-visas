...
import { ArrowRight, IdCard, AlertTriangle, CheckCircle2, Clock, DollarSign } from "lucide-react";
import SeoSchema from "@/components/SeoSchema";
import AuthorByline from "@/components/AuthorByline";
import EligibilityForm from "@/components/EligibilityForm";
import { getPrMaint, PRMAINT_LIST } from "@/data/prMaintenanceData";

const SITE = "https://www.4acesvisa.com";
const YEAR = new Date().getFullYear();

const PrMaintenancePage = () => {
  const { topic } = useParams<{ topic: string }>();
  const g = getPrMaint(topic);
  if (!g) return <Navigate to="/" replace />;

  const canonical = `${SITE}/pr-maintenance/${g.slug}`;
  const headline = `${g.name} (${YEAR})`;
  const metaTitle = `${g.shortName} ${YEAR} — Process, Fees & Timing | 4 Aces Visa`;
  ...
  const others = PRMAINT_LIST.filter((x) => x.slug !== g.slug);
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <IdCard className="h-3.5 w-3.5" /> PR Card & Travel
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/pr-maintenance/${g.slug}`} /></div>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto grid max-w-5xl gap-4 md:grid-cols-2">
          <Card><CardContent className="pt-6 space-y-3 text-sm">
            ...
            <div className="flex gap-2"><Clock className="h-4 w-4 text-primary mt-0.5" /><div><span className="font-semibold text-foreground">Timing: </span><span className="text-muted-foreground">{g.processingTime}</span></div></div>
            <div className="flex gap-2"><DollarSign className="h-4 w-4 text-primary mt-0.5" /><div><span className="font-semibold text-foreground">Fee: </span><span className="text-muted-foreground">{g.fee}</span></div></div>
          </CardContent></Card>
          <Card><CardContent className="pt-6 space-y-3 text-sm">
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 text-center">
              <h2 className="text-2xl font-bold text-foreground md:text-3xl">Protect your PR status</h2>
              <p className="mt-2 text-muted-foreground">Sahil Garg's team handles PR card renewals, PRTDs and residency obligation appeals every day.</p>
            </div>
            <EligibilityForm sourcePage={`pr-maintenance-${g.slug}`} heading={g.shortName} defaultValues={{ visa_type: "pr-card" }} />
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-foreground"><CheckCircle2 className="h-5 w-5 text-primary" /> More PR maintenance guides</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/pr-maintenance/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
...
export default PrMaintenancePage;