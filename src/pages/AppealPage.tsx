...
import { ArrowRight, Scale, AlertTriangle, CheckCircle2, Clock, DollarSign } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <Scale className="h-3.5 w-3.5" /> Appeal / Judicial Review
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/appeal/${g.slug}`} /></div>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto grid max-w-5xl gap-4 md:grid-cols-2">
          <Card><CardContent className="pt-6 space-y-3 text-sm">
            <div><span className="font-semibold text-foreground">Who can use it: </span><span className="text-muted-foreground">{g.who}</span></div>
            ...
            <div className="flex gap-2"><DollarSign className="h-4 w-4 text-primary mt-0.5" /><div><span className="font-semibold text-foreground">Cost: </span><span className="text-muted-foreground">{g.fee}</span></div></div>
          </CardContent></Card>
          <Card><CardContent className="pt-6 space-y-3 text-sm">
            <div><span className="font-semibold text-foreground">Forum: </span><span className="text-muted-foreground">{g.forum}</span></div>
            <div><span className="font-semibold text-foreground">Success rate: </span><span className="text-muted-foreground">{g.successRate}</span></div>
          </CardContent></Card>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 text-center">
              ...
              <p className="mt-2 text-muted-foreground">Sahil Garg's team handles appeals, judicial reviews and H&amp;C files. Book a free strategy call.</p>
            </div>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-foreground"><CheckCircle2 className="h-5 w-5 text-primary" /> Related appeal pathways</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/appeal/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
