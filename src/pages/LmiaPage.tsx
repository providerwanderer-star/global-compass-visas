...
import { ArrowRight, Briefcase, AlertTriangle, CheckCircle2, Clock, DollarSign } from "lucide-react";
import SeoSchema from "@/components/SeoSchema";
import AuthorByline from "@/components/AuthorByline";
import EligibilityForm from "@/components/EligibilityForm";
import { getLmia, LMIA_LIST } from "@/data/lmiaData";
...
const LmiaPage = () => {
  ...
  const g = getLmia(stream);
  if (!g) return <Navigate to="/" replace />;

  const canonical = `${SITE}/lmia/${g.slug}`;
  const headline = `${g.name} (${YEAR})`;
  const metaTitle = `${g.shortName} ${YEAR} — Eligibility, Fees & PR Path | Garg Brothers`;
  ...
  const others = LMIA_LIST.filter((x) => x.slug !== g.slug);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <Briefcase className="h-3.5 w-3.5" /> LMIA Stream
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/lmia/${g.slug}`} /></div>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto grid max-w-5xl gap-4 md:grid-cols-2">
          <Card><CardContent className="pt-6 space-y-3 text-sm">
            <div><span className="font-semibold text-foreground">Who qualifies: </span><span className="text-muted-foreground">{g.who}</span></div>
          ...
          <Card><CardContent className="pt-6 space-y-3 text-sm">
            <div><span className="font-semibold text-foreground">Recruitment: </span><span className="text-muted-foreground">{g.recruitment}</span></div>
            <div><span className="font-semibold text-foreground">PR pathway: </span><span className="text-muted-foreground">{g.prPath}</span></div>
          </CardContent></Card>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 pb-12">
        <div className="mx-auto max-w-4xl">
          <h2 className="mb-4 text-2xl font-bold text-foreground md:text-3xl">Step-by-step process</h2>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-foreground"><CheckCircle2 className="h-5 w-5 text-primary" /> Other LMIA streams</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/lmia/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
