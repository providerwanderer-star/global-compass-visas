...
import { ArrowRight, Languages, AlertTriangle, Clock, Wallet } from "lucide-react";
...
const FrancophonePathwayPage = () => {
  const { pathway: slug } = useParams<{ pathway: string }>();
  const p = getFrancophonePathway(slug);
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <Languages className="h-3.5 w-3.5" /> French / Francophone Pathway
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/francophone/${p.slug}`} /></div>
            <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Stat icon={<Languages className="h-4 w-4" />} label="Who qualifies" value={p.whoQualifies.split(".")[0]} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 text-2xl font-bold text-foreground">Other French pathways</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/francophone/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
...
export default FrancophonePathwayPage;
