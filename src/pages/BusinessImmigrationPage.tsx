...
import { getBusiness, BUSINESS_LIST } from "@/data/businessImmigrationData";
...
const BusinessImmigrationPage = () => {
  ...
  const g = getBusiness(program);
  if (!g) return <Navigate to="/" replace />;

  const canonical = `${SITE}/business-immigration/${g.slug}`;
  const headline = `${g.name} (${YEAR})`;
  const metaTitle = `${g.shortName} ${YEAR} — Investment, Timing & Process | 4 Aces Visa`;
  ...
  const others = BUSINESS_LIST.filter((x) => x.slug !== g.slug);
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="mx-auto max-w-4xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
              <Briefcase className="h-3.5 w-3.5" /> Business Immigration
            </div>
            ...
            <div className="mt-6"><AuthorByline articleHeadline={headline} articleUrl={`/business-immigration/${g.slug}`} /></div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="mx-auto max-w-3xl">
            <div className="mb-6 text-center">
              ...
              <p className="mt-2 text-muted-foreground">Sahil Garg's team prepares investor and entrepreneur applications end-to-end. Book a free consult.</p>
            </div>
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-foreground"><CheckCircle2 className="h-5 w-5 text-primary" /> More business immigration programs</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((o) => (
              <Link key={o.slug} to={`/business-immigration/${o.slug}`} className="group rounded-lg border border-border bg-card p-5 transition hover:border-primary hover:shadow-md">
